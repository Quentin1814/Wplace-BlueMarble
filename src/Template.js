...
export default class Template {
     ...
     * @since 0.65.2
     */
    constructor({ displayName = "My template", sortID = 0, authorID = "", url = "", file = null, coords = null, chunked = null, tileSize = 1000 } = {}) {
        ...
        this.pixels = []; // [{x, y, colorId}]
        this.paletteSize = Array.isArray(colorpalette)
            ? Math.max(...colorpalette.map((c) => c.id)) + 1 // taille suffisante pour l’array des restants
            : 300;
        ...
        this.allowedColorsSet = new Set(
            allowed.filter((c) => (c?.name || "").toLowerCase() !== "transparent" && Array.isArray(c?.rgb)).map((c) => `${c.rgb[0]},${c.rgb[1]},${c.rgb[2]}`)
        );
        ...
        this.rgbToMeta = new Map(
            allowed.filter((c) => Array.isArray(c?.rgb)).map((c) => [`${c.rgb[0]},${c.rgb[1]},${c.rgb[2]}`, { id: c.id, premium: !!c.premium, name: c.name }])
        );
    ...
    async createTemplateTiles() {
            ...
            for (let y = 0; y < imageHeight; y++) {
                for (let x = 0; x < imageWidth; x++) {
                    ...
                    if (!this.allowedColorsSet.has(key)) {
                        continue;
                    } // Skip non-palette colors (but #deface added to allowed)
                    required++;
        ...
        for (let pixelY = this.coords[3]; pixelY < imageHeight + this.coords[3]; ) {
            ...
            for (let pixelX = this.coords[2]; pixelX < imageWidth + this.coords[2]; ) {
                    ...
                    for (let x = 0; x < canvasWidth; x++) {
                        ...
                        if (imageData.data[pixelIndex] === 222 && imageData.data[pixelIndex + 1] === 250 && imageData.data[pixelIndex + 2] === 206) {
                            ...
                            if (!this.allowedColorsSet.has(key)) {
                                ...
                                // === NEW: indexer le pixel utile ===
                                const meta = this.rgbToMeta.get(key); // { id, ... }
                                if (meta && Number.isInteger(meta.id)) {
                                    // position locale du pixel "réel" (centre du bloc shreadSize x shreadSize)
                                    const localX = Math.floor(x / shreadSize);
                                    const localY = Math.floor(y / shreadSize);

                                    // coordonnées absolues sur la grille globale (tileX*1000 + offsetPx)
                                    const absX = this.coords[0] * this.tileSize + (pixelX + localX);
                                    const absY = this.coords[1] * this.tileSize + (pixelY + localY);

                                    this.pixels.push({ x: absX, y: absY, colorId: meta.id });
                                }
                            }
                        }
                    }
                ...
                const templateTileName = `${(this.coords[0] + Math.floor(pixelX / 1000)).toString().padStart(4, "0")},${(
                    this.coords[1] + Math.floor(pixelY / 1000)
                )
                    .toString()
                    .padStart(4, "0")},${(pixelX % 1000).toString().padStart(3, "0")},${(pixelY % 1000).toString().padStart(3, "0")}`;
    ...
    /**
     * Calcule les pixels restants par couleur en comparant au board.
     * @param {(x:number,y:number)=>number} boardColorAt - retourne l'id couleur du board à (x,y)
     * @returns {Uint32Array} remainingByColor
     */
    computeRemainingByColor(boardAt) {
        const remaining = new Uint32Array(this.paletteSize || 300);
        for (let i = 0; i < this.pixels.length; i++) {
            const p = this.pixels[i];
            if (!Number.isInteger(p.colorId)) continue;
            if (boardAt(p.x, p.y) !== p.colorId) remaining[p.colorId]++;
        }
        return remaining;
    }
}
